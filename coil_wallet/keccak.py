...
class Keccak:
    ...
    def keccakF(self, A, verbose=False):
        ...
        for i in range(self.nr):
            ...
            if verbose:
                self.printState(A,
                                "Satus end of round #%d/%d" % (i + 1, self.nr))
    ...
    def keccak256(self, input_bytes, r=1088, c=512, n=256, verbose=False):
        """Compute the Keccak[r,c,d] sponge function on message M

        input_bytes: data for hashing
        ...
        if (n % 8 != 0):
            raise KeccakError.KeccakError(
                'outputLength must be a multiple of 8')
        self.setB(r + c)
        ...
        # Padding of messages
        M = len(input_bytes) * 8, input_bytes.hex()
        ...
        for i in range((len(P) * 8 // 2) // r):
            Pi = self.convertStrToTable(P[i * (2 * r // 8):(i + 1) *
                                          (2 * r // 8)] + '00' * (c // 8))
